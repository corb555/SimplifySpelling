...
from spelling import Loc, Country, MatchScore, Normalize, QueryList
from spelling.GeoUtil import Query, Result, Entry, get_feature_group
...
class GeoSearch:
    ...
    def get_country_name(self, iso: str) -> str:
        ...
        # Returns:
            Country name or ''
        """
        self.logger.debug(f'Get country nm from iso [{iso}]')
        res = self._get_name(admin1_id='', admin2_id='', iso=iso, feature='ADM0')
        self.logger.debug(f'Found country nm=[{res}]')
        return res
    ...
    def get_alt_admin1_name(self, admin1_id, place: Loc) -> (str, str):
        """
        ...
        """
        self.logger.info(f'GET ADMIN1 ALT NAME {admin1_id}')
        if len(admin1_id) == 0:
            return '', ''
        ...
        if len(row_list) > 0:
            ...
            admin1_name, lang = self.get_alt_place_name(row[Entry.ID])
            self.logger.info(f'found geoid={row[Entry.ID]} name=[{admin1_name}]')
            if admin1_name == '':
                return '',''
    ...
    def get_alt_admin2_name(self, admin2_id, place: Loc) -> (str, str):
        """
             Get Admin1 name from alternate name table 
        ...
        # Returns:
            (admin1 name, lang)  place.admin2_id is updated with best match 
        """
        self.logger.info(f'GET ADMIN2 ALT NAME [{admin2_id}]')
        if len(admin2_id) == 0:
            return '', ''

        # Find GEOID for admin1_id and then get alternate name for GEOID
        row_list = []
        self._search(georow_list=row_list, place=None, name='', admin1_id='', admin2_id=admin2_id, iso=place.country_iso, feature='ADM1')
        if len(row_list) > 0:
            row = row_list[0]
            # Get alternate name for this GEOID
            admin2_name, lang = self.get_alt_place_name(row[Entry.ID])
            self.logger.info(f'found geoid={row[Entry.ID]} name=[{admin2_name}]')
            return admin2_name, lang
        else:
            return '', ''

    def get_alt_place_name(self, geoid) -> (str, str):
        """
        ...
        query_list = [
            Query(where="geoid = ? and lang = ?",
                  args=(geoid, self.lang),
        ...
        if len(row_list) > 0:
            return row_list[0][NAME], row_list[0][LANG]
        else:
            return '', ''
    ...
    @functools.lru_cache(maxsize=CACHE_SIZE)
    def get_admin1_idZZ(self, admin1_name, country_iso) -> (str, str):
        """
        ...
        best = self._search(georow_list=row_list, place=None, name=admin1_name, admin1_id='', admin2_id='', iso=country_iso, feature='ADM1', sdx='')
        ...
        if len(row_list) > 0:
            ...
            self.logger.debug(f'ADM1 found. ID={admin1_id} ISO={iso} {row_list}')
        else:
            admin1_id = ''
            iso = ''
            
        return admin1_id, iso
    ...
    def set_display_names(self, temp_place):
        ...
        res, lang = self.get_alt_place_name(temp_place.geoid)
        if res != '':  # and (lang == place_lang or lang == 'ut8'):
            temp_place.city = res

        res, lang = self.get_alt_admin1_name(temp_place.city, temp_place)
        if res != '':  # and (lang == place_lang or lang == 'ut8'):
            self.logger.debug(f'set adm1')
