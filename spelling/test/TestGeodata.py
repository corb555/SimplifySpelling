...
from spelling import GeoUtil, Spelling, Loc

halifax_lat = 44.646
bruce_cty_lat = 44.50009
albanel_lat = 48.91664
st_andrews_lat = 45.55614
halifax_name = "Halifax, Nova Scotia, Canada"
...
class TestGeodata(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        TestGeodata.geodata = Spelling.Geodata(directory_name=directory, display_progress=None,
        ...
        error = TestGeodata.geodata.open(repair_database=True)
        end_time = time.time()
    ...
    def run_lookup(self, title: str, entry: str):
        ...
        if len(self.place.georow_list) > 0:
            ...
            nm = f'{self.place.get_display_name(TestGeodata.geodata.geo_build.output_replace_dct)}'
            ...
            if self.place.result_type == Spelling.GeoUtil.Result.NOT_SUPPORTED:
                print(f'   NO match for {entry}:\n Country NOT SUPPORTED: {self.place.country_name} \n')
            else:
                print(f'   NO match for {entry}:\n')
    ...
    def test_place_name238(self):
        ...
        self.assertEqual("Baden-Württemberg, Germany", name, title)
    ...
    def test_place_name133(self):
        ...
        self.assertEqual("St George's, Hanover Square, London, England, United Kingdom", name, title)
    ...
    def test_parse00(self):
        ...
        print(title)
        self.place.parse_place(place_name="aaa,Banff,Alberta's Rockies,Alberta,Canada", geobuild=TestGeodata.geodata.geo_build)
        self.assertEqual("canada", self.place.country_name, title)
        
    def test_parse01(self):
        title = "***** Test Parse country"
        print(title)
        self.place.parse_place(place_name="Alberta's Rockies,Alberta,Canada", geobuild=TestGeodata.geodata.geo_build)
        self.assertEqual("canada", self.place.country_name, title)

    def test_parse02(self):
        title = "***** Test Parse country"
        print(title)
        self.place.parse_place(place_name="Alberta,Canada", geobuild=TestGeodata.geodata.geo_build)
        self.assertEqual("canada", self.place.country_name, title)
    ...
    def test_parse05(self):
        title = "***** Test Parse admin2"
        print(title)
        self.place.parse_place(place_name="Banff,Alberta's Rockies,Alberta,Canada", geobuild=TestGeodata.geodata.geo_build)
        self.assertEqual("alberta's rockies", self.place.admin2_name, title)
    ...
    def test_parse10(self):
        title = "***** Test Parse "
        print(title)
        self.place.parse_place(place_name="Banff,Alberta,Canada", geobuild=TestGeodata.geodata.geo_build)
        self.assertEqual("banff", self.place.city, title)

    def test_parse08(self):
        title = "***** Test Parse city with punctuation"
        print(title)
        self.place.parse_place(place_name="aaa,Banff!@#^&)_+-=;:<>/?,Alberta's Rockies,Alberta,Canada", geobuild=TestGeodata.geodata.geo_build)
        self.assertEqual("banff", self.place.city, title)

    def test_parse09(self):
        title = "***** Test Parse prefix"
        print(title)
        self.place.parse_place(place_name="pref,   abcde,Banff,Alberta's Rockies,Alberta,Canada", geobuild=TestGeodata.geodata.geo_build)
    ...
    def test_place_name04(self):
        title = "Halifax"
        lat, name = self.run_lookup(title, halifax_name)
        self.assertEqual(halifax_name, name, title)
    ...
    def test_place_name13(self):
        ...
        self.assertEqual("Old Bond Street, London, London, England, United Kingdom", name, title)
    ...
    def test_place_name20(self):
        ...
        self.assertEqual("Paris, Paris, Île-De-France, France",
                         name, title)
    ...
    def test_place_name28(self):
        ...
        self.assertEqual("Albanel, Québec, Canada", name, title)
    ...
    def test_place_name132(self):
        ...
        self.assertEqual("Hanover Square, London, England, United Kingdom", name, title)
