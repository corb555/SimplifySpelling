...
from spelling import GeoUtil, Normalize, ArgumentParserNoExit, GeoSearch
...
class Loc:
    ...
    def parse_place(self, place_name: str, geobuild):
        """
        ...
        """
        self.geobuild = geobuild
        self.geodb = geobuild.geodb   
        ...
        if token_count > 0:
            ...
            # Validate country
            self.country_iso = self.geodb.s.get_country_iso(self.country_name)  # Get Country country_iso
        ...
        if token_count > 1:
            ...
            if len(self.admin1_name) > 0:
                ...
                row_list = []
                self.admin1_id, iso = self.geodb.s.get_admin1_idZZ(self.admin1_name, self.country_iso)
                if self.admin1_id != '':
                    # Found Admin1
                    self.place_type = PlaceType.ADMIN1
                    self.georow_list = row_list
                    
                    #self.admin1_name = self.geodb.s.get_admin1_name(self.admin1_id, self.country_iso)
                    # self.logger.debug(f'adm1 nm=[{self.admin1_name}]\nGet ISO')
                    #self.logger.debug(f'2) Find iso for admin1 id [{self.admin1_id}] *******')

                    if iso != '':
                        self.country_iso = iso #self.geodb.s.get_iso_from_admin1_id(self.admin1_id, self.country_iso)
                        
                    self.logger.debug(f'adm1 id = {self.admin1_id} iso={self.country_iso}')

                    self.result_type = GeoUtil.Result.PARTIAL_MATCH
                    # Get country if blank
                    row_list = []
                    if self.country_name == '':
                        self.country_name = self.geodb.s.get_country_name(self.country_iso)
                else:
    ...
    def get_display_name(self, replace_dct) -> str:
        ...
        city, lang = self.geodb.s.get_alt_place_name(self.geoid)
        if city == '':
            city = self.city
        city = self.add_commas(city)
        
        adm2_nm, lg = self.geodb.s.get_alt_admin2_name(self.admin2_id, self)
        if adm2_nm == '':
            adm2_nm = self.admin2_name
        admin2 = self.add_commas(adm2_nm)
        
        adm1_nm, lg = self.geodb.s.get_alt_admin1_name(self.admin1_id, self)
        if adm1_nm == '':
            adm1_nm = self.admin1_name
        admin1 = self.add_commas(adm1_nm)
        
        self.logger.info(f'adm2 = [{self.admin2_name}]')
