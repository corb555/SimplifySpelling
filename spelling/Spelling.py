...
from spelling import GeoUtil, SpellBuild, Loc, MatchScore, GeoSearch
...
Provide a place lookup gazeteer based on files from geonames.org  
    ...
    def __init__(self, directory_name: str, display_progress,
        ...
        self.geo_build = SpellBuild.GeodataBuild(str(directory_name), display_progress=self.display_progress,
    ...
    def remove_duplicates(self, place):
        ...
        for geo_row in rows_sorted_by_latlon:
            ...
            # if country not supported, skip item
            if geo_row[GeoUtil.Entry.ISO] not in self.geo_build.supported_countries_dct:
                continue
    ...
    def open(self, repair_database: bool, query_limit=50):
        """
    ...
    def _verify_iso(self, iso):
        return iso in self.geo_build.supported_countries_dct
